...
Model Context Protocol server. Exposes Echo Memory as 14 MCP tools (`store`, `echo`, `memory_graph`, `memory_related`, `memory_get`, `entity_search`, `community_summaries`, `stats`, `forget`, `status`, `config_show`, `config_set`, `dump`, `persist`) via JSON-RPC 2.0 over stdio. Compatible with any MCP-aware AI client.
