...
/// Magic bytes identifying a Bellkis Echo Memory binary file.
...
pub fn save_binary(store: &EchoStore, path: &Path) -> Result<()> {
    ...
    let metadata_offset = file.stream_position()
        .map_err(|e| BellkisError::Persistence(format!("Failed to get stream position: {e}")))?;
    ...
    file.seek(SeekFrom::Start(0))
        .map_err(|e| BellkisError::Persistence(format!("Failed to seek: {e}")))?;
...
pub fn load_binary(path: &Path) -> Result<EchoStore> {
    ...
    let data = std::fs::read(path)
        .map_err(|e| BellkisError::Persistence(format!("Failed to read binary store: {e}")))?;

    if data.len() < HEADER_SIZE as usize {
        return Err(BellkisError::Persistence(format!(
    ...
    for i in 0..count_usize {
        ...
        for j in 0..dim_usize {
            ...
            let bytes: [u8; 4] = data[offset..offset + 4]
                .try_into()
                .map_err(|_| BellkisError::Persistence("Failed to read f32 bytes".into()))?;
    ...
    let metas: Vec<MemoryMeta> = serde_json::from_slice(meta_bytes)
        .map_err(|e| BellkisError::Persistence(format!("Failed to parse metadata: {e}")))?;
...
pub fn validate_binary(path: &Path) -> Result<bool> {
    ...
    let data = std::fs::read(path)
        .map_err(|e| BellkisError::Persistence(format!("Failed to read file for validation: {e}")))?;
...
fn parse_header(data: &[u8]) -> Result<([u8; 4], u32, u32, u64, u64, u32)> {
    if data.len() < HEADER_SIZE as usize {
        return Err(BellkisError::Persistence("Header too short".into()));
    }

    let magic: [u8; 4] = data[0..4]
        .try_into()
        .map_err(|_| BellkisError::Persistence("Failed to read magic".into()))?;

    let version = u32::from_le_bytes(
        data[4..8]
            .try_into()
            .map_err(|_| BellkisError::Persistence("Failed to read version".into()))?,
    );

    let dim = u32::from_le_bytes(
        data[8..12]
            .try_into()
            .map_err(|_| BellkisError::Persistence("Failed to read dim".into()))?,
    );
    ...
    let count = u64::from_le_bytes(
        data[16..24]
            .try_into()
            .map_err(|_| BellkisError::Persistence("Failed to read count".into()))?,
    );

    let metadata_offset = u64::from_le_bytes(
        data[24..32]
            .try_into()
            .map_err(|_| BellkisError::Persistence("Failed to read metadata offset".into()))?,
    );

    let crc = u32::from_le_bytes(
        data[32..36]
            .try_into()
            .map_err(|_| BellkisError::Persistence("Failed to read CRC".into()))?,
    );
...
mod tests {
    use super::*;
    use bellkis_core::SensitivityLevel;
